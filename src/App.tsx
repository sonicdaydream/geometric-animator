  ...
  const generateGIF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < frameCount; i++) {
        ...
        // canvasからフレームを追加
        gif.addFrame(canvas, { copy: true, delay: 100 }); // 100ms = 10fps
        
        setGifProgress(((i + 1) / frameCount) * 50); // 50%まで（フレーム生成）
      ...
      gif.on('progress', (p: number) => {
        setGifProgress(50 + (p * 50)); // 50%〜100%（GIF変換）
      });
      ...
      gif.on('finished', (blob: Blob) => {
        ...
        if (newWindow) {
          newWindow.document.write(`
            <html>
              <head><title>Generated GIF Animation - Right Click to Save</title></head>
              <body style="margin:0; background:#000; display:flex; flex-direction:column; justify-content:center; align-items:center; min-height:100vh; font-family:Arial;">
                <h2 style="color:white;">🎨 生成されたGIFアニメーション</h2>
                ...
                <div style="color:white; margin-top:20px; text-align:center;">
                  ...
                  <ol style="text-align:left; display:inline-block;">
                    ...
                    <li>ファイル形式: GIF（アニメーション対応）</li>
                    <li>サイズ: ${canvas.width}x${canvas.height}px</li>
                    <li>フレーム数: ${frameCount}フレーム (10fps)</li>
                  </ol>
                  <p><small>パターン: ${pattern} | 色彩: ${colorMode} | 品質: 高品質</small></p>
                  <button onclick="
                    const link = document.createElement('a');
                    link.download = 'geometric-animation-${pattern}-${colorMode}.gif';
                    link.href = '${url}';
                    link.click();
